...
// Load JSON and filter selected categories
$jsonData = file_get_contents(__DIR__ . '/jeopardy.json');
$allCategories = json_decode($jsonData, true)['categories'];
...
$selectedCategories = array_values($selectedCategories); // Reset keys to 0-4

// Get the question/answer for the selected category and value
$catIndex = (int)$_GET['cat'];
$value = (int)$_GET['val'];
$questionText = "Question not found.";
$correctAnswer = "answer";

if (isset($selectedCategories[$catIndex])) {
    foreach ($selectedCategories[$catIndex]['questions'] as $q) {
        if ($q['value'] == $value) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'skip') {
        ...
        $uTiles[] = $tKey;
        if (strcasecmp(trim($ans), $correctAnswer) === 0) {
