...
$allCategories = json_decode($jsonData, true)['categories']; // Decode into an array

// Check if categories are already saved in gamestate.txt
...
if (file_exists($gameStateFile)) {
    ...
    foreach ($gsLines as $line) {
        if (strpos($line, 'CATEGORIES=') === 0) {
            $selectedCategoryNames = explode(',', substr($line, 11)); // Extract saved categories
        }
    }
}

// If no categories are saved (new game), pick 5 random ones and save them
if (empty($selectedCategoryNames)) {
    shuffle($allCategories); // Randomize order
    $selectedCategories = array_slice($allCategories, 0, 5); // Take first 5
    $selectedCategoryNames = array_column($selectedCategories, 'name'); // Extract names
    // Append to gamestate.txt
    file_put_contents($gameStateFile, "CATEGORIES=" . implode(',', $selectedCategoryNames) . "\n", FILE_APPEND);
} else {
    // If categories are already saved, filter them from the JSON
    $selectedCategories = array_filter($allCategories, function ($cat) use ($selectedCategoryNames) {
        return in_array($cat['name'], $selectedCategoryNames);
    });
    $selectedCategories = array_values($selectedCategories); // Reset array keys to 0-4
}
...
<body>
    ...
    <div class="board-container">
        <table>
            ...
            for ($row = 0; $row < 5; $row++) {
                echo "<tr>";
                foreach ($selectedCategories as $catIndex => $category) {
                    $value = $vals[$row];
                    $tileKey = "$catIndex-$value";

                    if (in_array($tileKey, $usedTiles)) {
                        echo "<td>—</td>";
                    } else {
                        echo "<td><a class='tile-link' href='gamecard.php?cat=$catIndex&val=$value'>\$$value</a></td>";
                    }
                }
                echo "</tr>";
            }
