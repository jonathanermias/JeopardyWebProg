...
if (count($chosenArr) < 5) {
   
    $allIndices = range(0, count($allCategories) - 1);
    shuffle($allIndices);
    $chosenArr = array_slice($allIndices, 0, 5);
    file_put_contents($chosenFile, implode(',', $chosenArr));
}
$players = [];
...
<div class="board-container">
    <table>
        <tr>
            ...
            for ($i=0; $i<5; $i++) {
                $realCatIndex = $chosenArr[$i];
                $catName = $allCategories[$realCatIndex]['name'];
                echo "<th>" . htmlspecialchars($catName) . "</th>";
            }
            ?>
        </tr>
        <?php
